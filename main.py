...
def main(page: ft.Page):
    # EĞER KODDA BİR HATA OLURSA BEYAZ EKRANDA KALMASIN DİYE HATA YAKALAYICI EKLİYORUZ
    try:
        import datetime
        import urllib.request
        import json

        page.title = "Saha Paneli"
        ...
        FIREBASE_URL = "https://firestore.googleapis.com/v1/projects/kalibrasyon07-19e1d/databases/(default)/documents/saha_olcumleri"
        ...
        def verileri_gonder(e):
            ...
            btn_kaydet.content = ft.Text("Gönderiliyor...", size=16) 
            page.update()
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=10) as response:
                    if response.status == 200:
                        bildirim_goster("✅ Başarılı!", ft.Colors.GREEN_700)
                ...
                bildirim_goster(f"❌ Bağlantı Hatası: {ex}", ft.Colors.RED_700)
        ...
        btn_kaydet = ft.ElevatedButton(
            content=ft.Text("Sisteme Kaydet", size=16),
            ...
            controls=[
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Row([ch4, co2], spacing=10), ft.Row([o2, h2s], spacing=10)
                    ]), padding=12, bgcolor=ft.Colors.SURFACE_CONTAINER_HIGHEST, border_radius=10
                ),
                ft.Container(
                    content=ft.Column([
                        ft.Text("💨 Akış / Emiş", color=ft.Colors.WHITE70, size=14, weight=ft.FontWeight.BOLD),
                        ft.Row([emis]), ft.Row([debi1, debi2], spacing=10)
                    ]), padding=12, bgcolor=ft.Colors.SURFACE_CONTAINER_HIGHEST, border_radius=10
                ),
                ft.Container(
                    content=ft.Column([
        ...
        ana_tasarim_kutusu = ft.Container(content=mobil_ekran, width=420, padding=10)
        page.add(ana_tasarim_kutusu)
        page.update()

    except Exception as e:
        # EĞER ÇÖKERSE BEYAZ EKRAN YERİNE BURASI ÇALIŞACAK VE HATAYI KIRMIZI EKRANA BASACAK
        page.add(
            ft.Text("MAALESEF BİR HATA OLUŞTU!", color="red", size=20, weight=ft.FontWeight.BOLD),
            ft.Text(str(traceback.format_exc()), color="white", size=12, selectable=True)
        )
        page.update()
